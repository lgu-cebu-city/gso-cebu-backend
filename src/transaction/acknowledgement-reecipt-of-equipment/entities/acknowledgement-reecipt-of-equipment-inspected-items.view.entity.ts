...
@ViewEntity({
    ...
    FROM
      ...
      INNER JOIN request_for_inspection_item_details rfii
        ON(rfii.areId = arei.areId AND rfii.itemId = arei.id)
    `
})

export class AcknowledgementReceiptOfEquipmentInspectedItemsView {  
  @ViewColumn()
  areId: string
  @ViewColumn()
  parNo: string
